//https://github.com/skjeng/csharp_s7server/blob/master/Snap7Server/Program.cs
...
namespace LogoMqttBinding.Tests.Infrastructure
{
  ...
  public class S7Client
  {
    ...
    public int ReadMultiVars(ref S7DataItem Item, int ItemsCount) => Cli_ReadMultiVars(Client, ref Item, ItemsCount);

    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Cli_WriteMultiVars(IntPtr Client, ref S7DataItem Item, int ItemsCount);

    public int WriteMultiVars(ref S7DataItem Item, int ItemsCount) => Cli_WriteMultiVars(Client, ref Item, ItemsCount);
    ...
    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Cli_PlcCopyRamToRom(IntPtr Client, uint Timeout);

    public int PlcCopyRamToRom(uint Timeout) => Cli_PlcCopyRamToRom(Client, Timeout);

    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Cli_PlcCompress(IntPtr Client, uint Timeout);

    public int PlcCompress(uint Timeout) => Cli_PlcCompress(Client, Timeout);
  ...
  public class S7Server
  {
    ...
    public S7Server() => Server = Srv_Create();
    ...
    public int RegisterArea(int AreaCode, int Index, byte[] pUsrData, int Size) => Srv_RegisterArea(Server, AreaCode, Index, pUsrData, Size);

    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Srv_UnregisterArea(IntPtr Server, int AreaCode, int Index);

    public int UnregisterArea(int AreaCode, int Index) => Srv_UnregisterArea(Server, AreaCode, Index);
    ...
    public uint LogMask
    {
      ...
      {
        var Mask = new uint();
        if (Srv_GetMask(Server, mkLog, ref Mask) == 0)
          return Mask;
        return 0;
      }
      set => Srv_SetMask(Server, mkLog, value);
    }
    ...
    public uint EventMask
    {
      ...
      {
        var Mask = new uint();
        if (Srv_GetMask(Server, mkEvent, ref Mask) == 0)
          return Mask;
        return 0;
      }
      set => Srv_SetMask(Server, mkEvent, value);
    }
    ...
    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Srv_GetStatus(IntPtr Server, ref int CpuStatus, ref int ServerStatus, ref int ClientsCount);
  ...
  public class S7Partner
  {
    ...
    public int StartTo(
      ...
      ushort LocalTSAP,
      ushort RemoteTSAP)
      => Par_StartTo(Partner, LocalAddress, RemoteAddress, LocalTSAP, RemoteTSAP);
    ...
    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Par_BRecv(IntPtr Partner, uint Timeout, ref uint R_ID, byte[] Buffer, ref int Size);

    public int BRecv(uint Timeout, ref uint R_ID, byte[] Buffer, ref int Size) => Par_BRecv(Partner, Timeout, ref R_ID, Buffer, ref Size);
    ...
    [DllImport(S7Consts.Snap7LibName)]
    protected static extern int Par_GetStats(IntPtr Partner,
      ref int BytesSent,
      ref int BytesRecv,
      ref int SendErrors,
      ref int RecvErrors);
